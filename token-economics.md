...
| Skill registry/context lookup | 0–1,028 |
| Launch prompt + result envelope | 300–500 |
| **Total per delegation** | **~10,822–12,866** |
...
| Delegation | ~6,000T | 0–1 | ~6,000T |

Delegation recovery still benefits from passing compact artifact pointers (phase names, paths, IDs) instead of inlining artifact content. Even without external memory, pointer-style recovery is typically hundreds of tokens vs. thousands for full artifact replay.
...
| 6 | Parallel metadata reads | ~800T | Batch lightweight lookup calls before launch |
...
3. **Pass references, not content.** Each SDD artifact pointer is cheap to pass; inlining full artifact content can cost 3,000–15,000T per artifact.
