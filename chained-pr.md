...
description: >
  ...
  focus and stay within the 400-line cognitive review budget. Trigger: when a PR
  would exceed 400 changed lines, when planning chained PRs, stacked PRs, or
  reviewable slices.
license: MIT
metadata:
  version: "1.0"
...
The goal is not bureaucracy. The goal is preventing reviewer burnout so maintainers can review with care instead of skimming exhausted.
...
For any chain with more than two PRs, create a draft tracker PR before review starts. It must include:
...
6. Only merge the tracker PR into `main` after all children are merged.
...
## Chain Context Section (for PR body)
...
gh pr create --base feat/my-feature-01-core --title "feat(scope): next slice" --body-file pr-body.md
```
