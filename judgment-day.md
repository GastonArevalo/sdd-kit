...
1. Obtain the skill registry: read `.atl/skill-registry.md` from the project root → skip if none
...
3. Match relevant skills from the registry's **Compact Rules** by:
   ...
   - **Task context**: "review code" → framework/language skills; "create PR" → chained-pr skill
...
**If no registry exists**: warn the user ("No skill registry found — judges will review without project-specific standards. Run `/sdd-init` to generate the registry.") and proceed with generic review only.
...
The **orchestrator** (NOT a sub-agent) compares results after both delegates return:
...
**How to classify**: ask "Can a normal user, using the tool as intended, trigger this?" If YES → real. If it requires a malicious input, race condition in <1ms, or OS-specific edge case → theoretical.
...
│
▼
Resolve skills (Pattern 0): read .atl/skill-registry.md → match by code + task context → build Project Standards block
▼
...
- Fix 1: applied {list}
- Round 2: {N} issues remain → escalated
```
...
3. **MUST NOT** tell the user "done" until every JD reaches a terminal state (APPROVED or ESCALATED)
4. **After the Fix Agent returns**, the IMMEDIATE next action is re-launching judges in parallel for re-judgment
5. **When running multiple JDs in parallel**, each JD is independent — one completing does NOT allow skipping rounds on another

---

## Self-Check (before ANY terminal action)

Before pushing, committing, or telling the user "done":
...
**If ANY answer is "no"** → you skipped a step. Go back and complete it.
...
- Judges MUST be launched as async parallel delegates
